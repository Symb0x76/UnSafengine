...
#include <algorithm>
#include <cctype>
// Some configurations (notably VS Code IntelliSense / standalone header parsing)
// don't provide the Pin build defines (e.g. TARGET_IA32E/TARGET_WINDOWS).
// Define safe fallbacks only when missing so Pin headers parse correctly.
...
// Read a pointer-sized value from memory (ADDRINT-sized).
inline ADDRINT GetPTR(ADDRINT addr) {
#if defined(TARGET_IA32E)
	return static_cast<ADDRINT>(GetQWORD(addr));
#else
	return static_cast<ADDRINT>(GetDWORD(addr));
#endif
}
...
// Write a pointer-sized value to memory (ADDRINT-sized).
inline void PutPTR(ADDRINT addr, ADDRINT val) {
#if defined(TARGET_IA32E)
	PutQWORD(addr, static_cast<UINT64>(val));
#else
	PutDWORD(addr, static_cast<UINT32>(val));
#endif
}
