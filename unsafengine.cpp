// IntelliSense often doesn't inherit the Pin build defines (e.g. TARGET_IA32E/TARGET_WINDOWS),
// which can break Pin headers parsing and cascade into many false errors.
// Provide safe fallbacks for the editor only.
#if defined(__INTELLISENSE__)
#if !defined(TARGET_IA32) && !defined(TARGET_IA32E)
#define TARGET_IA32E 1
#endif
#if defined(_WIN32) && !defined(TARGET_WINDOWS) && !defined(TARGET_LINUX) && !defined(TARGET_MAC)
...
// prepare disassmbly
// XED decode mode depends on target architecture.
...
void* MakeImportSection(UINT32* size, UINT32* idt_size, UINT32 vloc)
{
    ...
    *fout << "# Import Directory Table at " << toHex(reinterpret_cast<ADDRINT>(iid)) << endl;
    ...
    for (auto& [iat_elem_addr, func_addr, dll_name, func_name] : iat) {
        ...
        // Process ordinal function		
            ...
            ilt_val |= static_cast<ADDRINT>(IMAGE_ORDINAL_FLAG32);
#else
            ilt_val |= static_cast<ADDRINT>(IMAGE_ORDINAL_FLAG64);
#endif
        ...
        // Name function
            ...
            PutPTR(ilt, ilt_val);
            // iat_elem_addr is an absolute address in the target image (see FindIAT).
            PIN_SafeCopy((void*)iat_elem_addr, (const void*)&ilt_val, ADDRSIZE);
            ilt += ADDRSIZE;
...
void FixInstructionAtOEP() {
    // This patch is sample/x64-specific (writes bytes for "sub rsp, 28").
    // Disable it for x86 builds to avoid corrupting code.
#if defined(TARGET_IA32E)
...
void ResolveObfuscatedCalls() {
    ...
    for (auto const& [addr, obfuscated_target, original_target, call_type] : obfuscated_calls) {
        ...
        *popcode = opcode;
        ...
        // Encoding differs:
        // - x64: FF 15/25 disp32 is RIP-relative.
        // - x86: FF 15/25 imm32 is absolute memory address.
#if defined(TARGET_IA32E)
        int32_t relative_address = static_cast<int32_t>(indirect_target - (addr + 6));
        int32_t* preladdr = XED_STATIC_CAST(xed_int32_t*, addr + 2);
        *preladdr = relative_address;
#else
        uint32_t absolute_address = static_cast<uint32_t>(indirect_target);
        uint32_t* pimmaddr = XED_STATIC_CAST(xed_uint32_t*, addr + 2);
        *pimmaddr = absolute_address;
#endif
    }
}
    ...
    *fout << "# Searching for IAT in .text section" << endl;
    ...
    while (ea0 < main_text_section_end_address) {
        ...
        for (; non_function_count < 2; idx1 += ADDRSIZE, ea1 += ADDRSIZE) {
            ADDRINT fn_addr = *XED_STATIC_CAST(const ADDRINT*, ea1);
            if (fn_addr == 0) {
                non_function_count++;
                iat.push_back({ ea1, 0, "", "" });
                continue;
...
VOID MemReadAnalysis(THREADID tid, ADDRINT ins_addr, ADDRINT mem_addr, UINT32 mem_sz) {
    if (!IsMainImage(mem_addr)) return;
